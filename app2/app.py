...
def get_data():
    ...
    cache_key = f"data:app2:{category}"
    hostname  = socket.gethostname()
    ...
    if r and not nocache:
        try:
            ...
            if cached:
                ...
                data["served_by"]      = f"app2 ({hostname})"
...
def static_asset():
    ...
    payload = {
        ...
        "size_kb":       256,
        "origin_server": f"app2 ({hostname})",
        "generated_at":  time.strftime("%Y-%m-%d %H:%M:%S"),
        "data":          "x" * 1024,
    }
    resp = jsonify(payload)
    resp.headers["Cache-Control"] = "public, max-age=60"
    resp.headers["X-Origin-Server"] = f"app2"
    return resp
...
def stress():
    ...
    hostname = socket.gethostname()
    return f"""
    <h1>Stress Test Dimulai - Server 2</h1>
    <hr>
    <p><b>Server:</b> app2</p>
    <p><b>Hostname:</b> {hostname}</p>
