...
def get_data():
    ...
    Query params:
      ?category=<string>  → Kategori data (default: "general")
      ?nocache=1          → Bypass cache (paksa ambil dari server)
    """
    category = request.args.get("category", "general")
    nocache  = request.args.get("nocache", "0") == "1"
    cache_key = f"data:app1:{category}"
    hostname  = socket.gethostname()
    ...
    if r and not nocache:
        try:
            ...
            if cached:
                ...
                data["served_by"]      = f"app1 ({hostname})"
    ...
    if r:
        try:
            ...
            source = "server (computed + cached)"
        except Exception:
            source = "server (computed, cache write failed)"
...
def static_asset():
    ...
    payload = {
        ...
        "size_kb":       256,
        "origin_server": f"app1 ({hostname})",
        "generated_at":  time.strftime("%Y-%m-%d %H:%M:%S"),
        "data":          "x" * 1024,  # Simulasi payload
    }
    resp = jsonify(payload)
    # Header untuk Nginx proxy_cache
    resp.headers["Cache-Control"] = "public, max-age=60"
    resp.headers["X-Origin-Server"] = f"app1"
    return resp
